...
| [`mfrm_app/guidance.py`](../mfrm_app/guidance.py) and the first-run adapter | Keep the five stable node IDs and pure reducer authoritative; retire the dormant static tutorial only after every node has an exact registered Help link/return target | Bilingual skip/exit/resume/restart/formative journeys preserve AnalysisID and Help return restores the same node/card |
