"""Run the frozen visible-window v2 FACETS launcher qualification."""
...
PLAN_PATH = ROOT / "validation" / "facets_visible_launcher_qualification_v2_plan_20260811.json"
OUTPUT_DIR = ROOT / "validation" / "facets_visible_launcher_qualification_v2_20260811"


def main() -> None:
    if OUTPUT_DIR.exists():
        raise FileExistsError(f"Refusing to overwrite v2 launcher qualification: {OUTPUT_DIR}")
    plan = json.loads(PLAN_PATH.read_text(encoding="utf-8"))
    dependencies = (
        ("v1 result", ROOT / plan["v1_result"], plan["v1_result_sha256"]),
    ...
    temporary = Path(tempfile.mkdtemp(prefix="facets_visible_qualification_v2_"))
    ...
    result = {
        "schema_version": "facets_visible_launcher_qualification_v2_result_v1",
        "pass": passed,
