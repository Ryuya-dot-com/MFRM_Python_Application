#!/usr/bin/env Rscript
# Independent scalar weighted-PCM formula; no Python affine design is consumed.
...
stopifnot(length(idx$score_k) == 147L, all(idx$score_k %in% 0:3),
          all(is.finite(idx$weight)), all(idx$weight >= 0), sum(idx$weight) == 141.5,
...
continuous <- function(par, settings) {
  ...
  stopifnot(max(abs(eap[1:2]-input$x[1:2]*par[1])) < 1e-9, max(abs(sd[1:2]-sigma)) < 1e-9)
