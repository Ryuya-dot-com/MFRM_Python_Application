#!/usr/bin/env Rscript
# Independent scalar PCM check for arbitrary N, complete 4 x 5 crossing.
# Adapt the historical evaluator without editing its hash-bound source.
args <- commandArgs(TRUE)
stopifnot(length(args) == 3L)
input <- jsonlite::read_json(args[1], simplifyVector = TRUE)
cases <- jsonlite::read_json(args[2])
stopifnot(identical(input$classification, "DEVELOPMENT_ONLY"),
          !input$scientific_inference_ready, !input$qualification_eligible)
data <- input$data
n <- length(unique(data$Person))
stopifnot(nrow(data) == 20L * n, all(data$Score %in% 0:4),
          identical(sort(unique(data$Person)), sprintf("P%02d", seq_len(n))),
...
finite_summary <- function(x, rule) {
  ...
  deviations <- matrix(theta, n, length(theta), byrow = TRUE) - eap
...
for (settings in input$integration |> split(seq_len(nrow(input$integration)))) {
  ...
  stopifnot(abs(control$nll - nrow(data) * log(5)) < 1e-8,
